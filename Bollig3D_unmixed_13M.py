...
# Boundary pressure
P_target = (6 * rho_target)**(4.0/3) # P propto (S rho)^4/3, ignoring the constant factors
...
for v_in in initial_vs:
    ...
    f = open('Bollig3D_unmixed_13M.txt' ,'w')
    z = z_s
    ...
    for p in range (100000):
        ...
        bending_factor = 1.0 - (1.0 - ((r_in_nat / r)**2.0 / GR_factor_angle**2.0))**0.5 # gravitational bending
        qdot = ((3.4e-24 * ((L_nuebar * e_nuebar**2.0 + L_nue * e_nue**2.0) * GR_factor_1**6.0 * (1.0e+6 / r_in)**2) * bending_factor * GR_factor_angle**6.0
...
f = np.loadtxt('Bollig3D_unmixed_13M.txt')
plt.loglog(f[:, 0], f[:, 1])
plt.show()
plt.loglog(f[:, 1], f[:, 3])
plt.show()
