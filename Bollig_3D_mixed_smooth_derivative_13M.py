    ...
    for p in range (100000):
        ...
        if(r / 5.0e+10 < R_t):
            ...
            if T : 
                ...
                #if m > 1.2: 
                    #print(((2 * vs**2 / r) - ((GM / r**2) * (1 - vs**2) * GR_factor) - (qdot * beta / (v * y_fac * (1 + 3 * vs**2)))) * (1.0 - v**2.0))

                transition_start = 0.85
                transition_end = 1.15
                regularizer = 1e-6

                if (m > 0.97 and m < 1.03): # step over the critical radius
                    dv = dv_temp
                    dvs = dvs_temp
                else:
