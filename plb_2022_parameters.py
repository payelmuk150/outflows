...
while True:
    ...
    # Initial temperature
    T_in = sc.optimize.fsolve(initials, 4.8)[0]
    Ye = 0.5
    print('T_in',T_in)
    Ye_list = [Ye]
    print('Initial Ye', Ye)
    ...
    for p in range (1000000):
        ...
        if(r / 5.0e+10 < R_t):
            ...
            if T : 
                ...
                #dvs = ((qdot * dr_nat / v) - (v * dv) - (GM * dr_nat / r**2)) / (6 * vs)
                fac_beta = 1 + ((1.0/4) * T * derivative_A_term)
                dT = (((qdot * fac_beta * dr_nat / v) - (v * dv) - (GM * dr_nat / r**2))/fac_beta - (T*dS/m_n)) * m_n/S
